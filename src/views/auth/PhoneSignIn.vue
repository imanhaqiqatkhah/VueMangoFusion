            ...
            <div v-else-if="step === 2">
              <form @submit.prevent="verifyCode">
                ...
                <div class="d-flex gap-2">
                  ...
                  <button :disabled="isLoading" type="submit" class="btn btn-primary w-50">
                    <span v-if="isLoading" class="spinner-border spinner-border-sm ms-2"></span>
                    تأیید و ورود
                  </button>
                </div>
...
const sendVerificationCode = async () => {
  ...
  try {
    await smsService.sendLoginCode(phoneNumber.value)
    step.value = 2
    startCountdown()
  } catch (error) {
...
const verifyCode = async () => {
  ...
  try {
    // TODO: این قسمت رو بعداً تکمیل می‌کنیم
    console.log('Verifying code:', verificationCode.value)
    errorList.push('قسمت تأیید کد هنوز پیاده‌سازی نشده است')
  } catch (error) {
